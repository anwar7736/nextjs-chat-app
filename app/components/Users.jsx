...
const Users = ({ users }) => {
  ...
  return (
    <div>
      {
        users?.map(user => {
          return  (<div key={user?._id} className={`${selectedUser == user?._id ? 'bg-gray-100' : ''} flex items-center mb-4 cursor-pointer hover:bg-gray-100 p-2 rounded-md`} onClick={() => getMessages(user)} >
